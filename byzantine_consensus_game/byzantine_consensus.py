...
class ByzantineConsensusGame:
    ...
    def get_honest_termination_votes(self, agent_votes: Dict[str, Optional[bool]]) -> tuple[int, int]:
        """
        Count termination votes from honest agents only.
        
        Args:
            agent_votes: Dict mapping agent_id to vote (True=stop, False=continue, None=abstain)
            
        Returns:
            Tuple of (votes_to_stop, total_honest_agents)
        """
        honest_stop_votes = sum(
            1 for agent_id, vote in agent_votes.items()
            if not self.agents[agent_id].is_byzantine and vote is True
        )
        total_honest = sum(1 for agent in self.agents.values() if not agent.is_byzantine)
        return honest_stop_votes, total_honest
    
    def should_terminate_by_vote(self, agent_votes: Dict[str, Optional[bool]]) -> bool:
        """
        Check if game should terminate based on honest agent votes.
        Requires supermajority (2/3) of honest agents to vote STOP.
        
        Args:
            agent_votes: Dict mapping agent_id to vote (True=stop, False=continue, None=abstain)
            
        Returns:
            True if ≥2/3 of honest agents vote to stop
        """
        honest_stop_votes, total_honest = self.get_honest_termination_votes(agent_votes)
        if total_honest == 0:
            return False
        # Require supermajority: 2/3 (≈66.7%) of honest agents
        supermajority_threshold = (2 * total_honest) / 3
        will_terminate = honest_stop_votes >= supermajority_threshold
    ...
    def advance_round(self, agent_votes: Dict[str, Optional[bool]] = None):
        ...
        # Check if agents voted to terminate (only honest votes count)
        if agent_votes and self.should_terminate_by_vote(agent_votes):
            # Supermajority of honest agents voted to stop - game ends
            if VERBOSE:
